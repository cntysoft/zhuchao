 ...
 * 图片选择器
 */
Ext.define('App.Yunzhan.CmMgr.Lib.FieldWidget.ImageGroup', {
   extend: 'App.Yunzhan.CmMgr.Lib.FieldWidget.AbstractField',
   requires: [
      'App.Yunzhan.CmMgr.Comp.IconView',
      'Ext.menu.Menu',
      'WebOs.Component.Uploader.Window',
      'App.Yunzhan.CmMgr.Comp.InternetPicWin',
    ...
    *
    * @private
    * @property {App.Yunzhan.CmMgr.Lib.FieldWidget.InternetPicWindow} internetPicWinRef
    */
   ...
   /**
    * @private
    * @property {App.Yunzhan.CmMgr.Comp.IconView} iconViewRef
    */
   iconViewRef : null,
   ...
   getInternetPicWindow : function()
   {
      if(null == this.internetPicWinRef){
         this.internetPicWinRef = new App.Yunzhan.CmMgr.Comp.InternetPicWin({
            listeners : {
   ...
   recursiveSaveRemoteImageHandler : function(queue)
   {
      if(queue.length > 0){
         ...
         this.editorRef.callSaverApi('downloadRemoteFile', {
            fileUrl : current,
            useOss: true,
            targetDir: '/Data/UploadFiles/Apps/Yunzhan/Content',
         }, function(response){
            //没成功就不管了 很可能是网络错误
            if(response.status){
               //替换文件内容
      ...
      Ext.require('WebOs.Component.Uploader.Window', function(){
         ...
         if(null == this.uploadWinRef){
            Ext.apply(uploaderConfig, {
               initUploadPath : ZC.getAppUploadFilesPath('Yunzhan', 'Content'),
               uploaderConfig : {
                  //启用附件追踪
                  enableFileRef : true,
