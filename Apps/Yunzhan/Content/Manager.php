...
use App\Yunzhan\Content\Constant as CONT_CONST;
use App\Yunzhan\CmMgr\Model\General as GeneralModel;
use Cntysoft\Kernel\App\AbstractLib;
use Cntysoft\Kernel;
use App\Yunzhan\CmMgr\Constant as CMM_CONST;
/**
 * 信息管理类，提供基本的信息操作接口
 */
class Manager extends AbstractLib
{
    ...
    *
    * @param int $id
    * @return \App\Yunzhan\CmMgr\Model\General
    */
   ...
   public function add($cmid, array $data)
   {
      ...
      $cmm = $this->getAppCaller()->getAppObject('Yunzhan', 'CmMgr', 'Mgr');
      ...
      $nid = $data['nodeId'];
      if (!$this->getAppCaller()->call('Yunzhan', 'Category', 'Structure', 'hasNode', array($nid))) {
         $errorType = $this->getErrorType();
      ...
      //判断信息是否存在
      $isExist = $this->getAppCaller()->call('Yunzhan', 'Content', 'InfoList', 'titleExist', array($cmid, $nid, $data['title']));
      if ($isExist) {
         $errorType = $this->getErrorType();
         Kernel\throw_exception(new Exception(
   ...
   public function update($id, array $data)
   {
      ...
      if (isset($gdata['title']) && $gdata['title'] !== $gmodel->getTitle()) {
         ...
         $isExist = $this->getAppCaller()->call('Yunzhan', 'Content', 'InfoList', 'titleExist', array($cmid, $nodeId, $gdata['title']));
         if ($isExist) {
            $errorType = $this->getErrorType();
            Kernel\throw_exception(new Exception(
