...
class Acl extends AbstractLib
{
    ...
    */
   public function register($phone, $name, $password, $smsCode)
   {
      ...
      return $this->getAppCaller()->call(
                      Constant::MODULE_NAME, Constant::APP_NAME, Constant::APP_API_MANAGER, 'addProvider', array($userInfo)
      );
   }
   ...
   public function getPicCodeSessionKey($type)
   {
      switch ($type) {
         case Constant::PIC_CODE_TYPE_REG:
            return \Cntysoft\PROVIDER_USER_S_KEY_REG_CHK_CODE;
         case Constant::PIC_CODE_TYPE_FORGET:
            return \Cntysoft\PROVIDER_USER_S_KEY_FORGET_CHK_CODE;
         case Constant::PIC_CODE_TYPE_LOGIN:
            return \Cntysoft\PROVIDER_USER_S_KEY_LOGIN_CHK_CODE;
         default:
            return \Cntysoft\PROVIDER_USER_S_KEY_REG_CHK_CODE;
      }
   }
   ...
   public function sendSmsCode($phone, $type = Constant::SMS_TYPE_REG)
   {
      if (!in_array($type, array(
         ...
         $errorType = $this->getErrorType();
         Kernel\throw_exception(new Exception(
                 $errorType->msg('E_PROVIDER_ACL_SMS_TYPE_ERROR', $type), $errorType->code('E_PROVIDER_ACL_SMS_TYPE_ERROR')
      ...
      if (0 != $ret['code']) {
         $errorType = $this->getErrorType();
         Kernel\throw_exception(new Exception(
                 $errorType->msg('E_PROVIDER_ACL_SMS_SEND_ERROR'), $errorType->code('E_PROVIDER_ACL_SMS_SEND_ERROR')
                 ), $this->getErrorTypeContext());
      }
   ...
   public function getCookieKeys()
   {
      if (null == $this->cookieKeys) {
         ...
         if (isset($config->cookieKeys)) {
            $this->cookieKeys = $config->cookieKeys->provider->toArray();
         } else {
    ...
    */
   public function login($key, $password, $type = Constant::FRONT_USER_PHONE_LOGIN, $chkCode = null, $remember = false)
   {
      //如果传入了验证码，首先验证登录的验证码
      if ($chkCode) {
      ...
      if (!$user) {
         $errorType = $this->getErrorType();
         Kernel\throw_exception(new Exception(
                 $errorType->msg('E_PROVIDER_NOT_EXIST', $key), $errorType->code('E_PROVIDER_NOT_EXIST')
                 ), $this->getErrorTypeContext());
      }
      if (Constant::PROVIDER_STATUS_LOCK == $user->getStatus()) {
         $errorType = $this->getErrorType();
         Kernel\throw_exception(new Exception(
                 $errorType->msg('E_PROVIDER_STATUS_ERROR'), $errorType->code('E_PROVIDER_STATUS_ERROR')
                 ), $this->getErrorTypeContext());
      }
      try {
         ...
         //清空当前登录错误的次数
         $user->setCurrentLoginErrorTimes(0);
         $user->save();
         ...
         //登陆失败，记录信息
         $loginErrorTimes = $user->getLoginErrorTimes();
         $currentLoginErrorTimes = $user->getCurrentLoginErrorTimes();
         $user->setLoginErrorTimes($loginErrorTimes + 1);
         $user->setCurrentLoginErrorTimes($currentLoginErrorTimes + 1);
         $user->save();
   ...
   public function getCurUser()
   {
      if (null == $this->curUser) {
         ...
         if (empty($data)) {
            $errorType = $this->getErrorType();
            Kernel\throw_exception(new Exception(
                    $errorType->msg('E_PROVIDER_COOKIE_NOT_EXIST'), $errorType->code('E_PROVIDER_COOKIE_NOT_EXIST')
                    ), $this->getErrorTypeContext());
         }
         $data = explode('|', $data);
         $key = $data[0];
   ...
   public function loginByCookie()
   {
      ...
      try {
         ...
         //验证用户的状态
         if ($user && Constant::PROVIDER_STATUS_LOCK == $user->getStatus()) {
            $this->logout();
            $errorType = $this->getErrorType();
            Kernel\throw_exception(new Exception(
                    $errorType->msg('E_PROVIDER_STATUS_ERROR'), $errorType->code('E_PROVIDER_STATUS_ERROR')
            ...
            $errorType = $this->getErrorType();
            Kernel\throw_exception(new Exception(
                    $errorType->msg('E_PROVIDER_NOT_EXIST'), $errorType->code('E_PROVIDER_NOT_EXIST')
                    ), $this->getErrorTypeContext()
            );
         }
         //登录成功
