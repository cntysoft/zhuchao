...
class FeedbackMgr extends AbstractLib
{
    ...
    * @return type
    */
   public function getFeedbackList(array $cond, $total = false, $orderBy = 'inputTime DESC', $limit = 15, $offset = 0)
   {
      $bind = array(
         'order' => $orderBy,
         'limit' => array(
      ...
      $feedbacks = FeedbackModel::find($condition);
      if ($total) {
         return array(
            'total' => FeedbackModel::count($cond),
            'item'  => $feedbacks
         );
      }
      return $feedbacks;
