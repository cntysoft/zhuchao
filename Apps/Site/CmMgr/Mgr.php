...
use App\Site\CmMgr\Model\Content as ContentModel;
use App\Site\CmMgr\Model\Fields as FieldsModel;
...
class Mgr extends AbstractLib
{
    ...
    *
    * @param int $id
    * @return \App\Site\CmMgr\Model\Fields
    */
   ...
   public function deleteField($id)
   {
      ...
      if(!in_array($modelKey, self::$buildInMKeys)){
         //内置的不生成吧
         $this->generateModelClassFile($this->getSiteId(), $modelKey, array()); //统一处理
      }
    ...
    * 将相应的字段添加到数据库中
    *
    * @param \App\Site\CmMgr\Model\Content $model
    * @param array $fields
    */
    ...
    *
    * @param string $modelKey
    * @param \App\Site\CmMgr\Model\Fields $field
    */
   ...
   /**
    * @see App\Site\CmMgr::setupBuildInModelFields();
    * 这个函数内置的指定的路径
    */
   ...
   protected function emptyModels(array $models)
   {
      ...
      Kernel\truncate_table('app_site_cmmgr_cmodel');
      Kernel\truncate_table('app_site_cmmgr_cmodel_fields');
    ...
    *
    * @param int $mid
    * @return \App\Site\CmMgr\Model\Content
    */
...
//      $modelEmpty = $appManager->call('Cms', 'ContentManager', 'InfoList/cmodeInfoEmpty', array($id));
//      if (!$modelEmpty) {
//         $errorType = $this->getErrorType();
//         Kernel\throw_exception(new Exception(
//            $errorType->msg('MODEL_INFO_NOT_EMPTY', $id), $errorType->code('MODEL_INFO_NOT_EMPTY')), $errorType);
//      }
      $db = Kernel\get_db_adapter();
      ...
      try {
         ...
         $this->getAppCaller()->call(
            \App\Site\Category\Constant::MODULE_NAME,
            \App\Site\Category\Constant::APP_NAME,
            \App\Site\Category\Constant::APP_API_STRUCTURE,
            'deleteC2MapByModel',
            array(
               $model->getId()
            )
   ...
   protected function generateModelClassFile($modelKey, $fields)
   {
      ...
      $fileGen->setNamespace('App\Site\CmMgr\Model');
...
TPL;
   ...
   {
      return 'App\Site\CmMgr\Model'.'\\'.$mkey;
   }
   ...
   {
      return \Cntysoft\Kernel\StdDir::getAppRootDir('Site', 'CmMgr') .DS .'Js'. DS  . 'Editor' .DS. $editor . '.js';
   }
   ...
   {
      return \Cntysoft\Kernel\StdDir::getAppRootDir('Site', 'Content').DS.'Saver'.DS.$saver.'.php';
   }
