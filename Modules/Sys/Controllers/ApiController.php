...
class ApiController extends AbstractController
{
   ...
   protected function authorizeRequest($type, array $meta)
   {
      ...
      $gate = $di->get('SysGate');
      if (!in_array($key, $allows)) {//不在白名单中，判断是否登录
         $gate->isLogin();
      }

      //判断当前用户所处的用户组是否授权
//      if (\Cntysoft\API_CALL_APP == $type) {
//         $user = $gate->getCurUser();
//         $permMgr = $di->get('SysPermMgr');
//         $permMgr->checkUserApiPermission($user, $invokeMeta['module'], $invokeMeta['name'], $invokeMeta['method']);
//      }
   }
