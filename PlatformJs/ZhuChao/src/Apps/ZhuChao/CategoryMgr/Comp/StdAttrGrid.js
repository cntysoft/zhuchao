...
Ext.define('App.ZhuChao.CategoryMgr.Comp.StdAttrGrid', {
   ...
   alias : 'widget.shopcategorymgrcompstdattrgrid',
   mixins : {
   ...
   saveColorAttrRequestHandler : function (data, mode)
   {
      if(mode == ZhuChao.Const.NEW_MODE){
         //判断是否存在
         var store = this.customColorRef.store;
         if(!store.findRecord('color', data.color, 0, false, false, true)){
            store.add(data);
         }
      } else if(mode == ZhuChao.Const.MODIFY_MODE){
         var record = this.customColorRef.store.getById(data.id);
         for(var key in data) {
...
Ext.define('App.ZhuChao.CategoryMgr.Comp.StdAttrWindow', {
   ...
   {
      var L = this.LANG_TEXT.FIELDS, MSG = this.LANG_TEXT.MSG;
      return {
         xtype : 'form',
         bodyPadding : 10,
         defaults : {
         ...
         items : [{
               ...
               validator: function (val) {
                  var valiate = val.replace(/[0-9]/g, '');
                  return valiate ? true : MSG.NOT_ALL_NUMBER;
               }
            }, {
   ...
   saveHandler : function ()
   {
      ...
      if(form.isValid()){
         ...
         if(ZhuChao.Const.MODIFY_MODE == this.mode){
