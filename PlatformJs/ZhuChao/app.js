...
Ext.define('ZC', {
    ...
    singleton : true,
    requires : [
        'ZhuChao.PackageRequires',
        'ZhuChao.ExtJsRequires',
        'ZhuChao.Kernel.StdPath',
        'ZhuChao.Lang.zh_CN',
        'ZhuChao.Kernel.Const',
        'ZhuChao.Kernel.Component.Uploader.SimpleUploader',
        'ZhuChao.Os.SysMenuHandler',
        'ZhuChao.Os.DesktopMenuHandler',
        'ZhuChao.Utils'
    ],
    /**
     * 系统开始加载时间， 暂时还没有使用
    ...
    langCls : [
        ...
        'ZhuChao.Lang.' + window.CNTYSOFT_LANG
    ],
    /**
    ...
    initLang : function ()
    {
        ...
        var ZhuChaoLang = Ext.create(this.langCls[1]);
        var alias = Ext.Function.alias;
        Cntysoft.LangManager.register('System', sysLang);
        Cntysoft.LangManager.register('ZhuChaoLang', ZhuChaoLang);
        Cntysoft.GET_LANG_TEXT = alias(sysLang, 'getText');
        this.GET_LANG_TEXT = alias(ZhuChaoLang, 'getText');
    },
    run : function ()
    {
        this.callParent();
    ...
    getCdnServerInfo : function ()
    {
        ...
        var sysSetting = this.sysEnv.get(ZhuChao.Kernel.Const.ENV_SYS_SETTING);
    ...
    {
        var handler = new ZhuChao.Os.SysMenuHandler(menu);
        menu.add(handler.getMenuItems());
    },
    desktopMenuRequestHandler : function (menu)
    {
        var handler = new ZhuChao.Os.DesktopMenuHandler(menu);
        menu.add(handler.getMenuItems());
    }
}, function (){
    //给ZhuChao名称空间加上一些东西
